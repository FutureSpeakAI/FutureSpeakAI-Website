...
-   **Voice Agent (Friday)**: Gemini 2.5 Flash native audio voice agent via WebSocket proxy (`server/gemini-live.ts`), model `models/gemini-2.5-flash-native-audio-latest`. Features: Kore voice, single-phase connection (greeting via `clientContent` on setup, then same WS stays open for bidirectional audio — no reconnection needed for normal flow), automatic VAD (do NOT send `activityStart`/`activityEnd` — causes immediate 1007 disconnect), function calling for name capture (`saveUserName`), returning-user recognition (`confirmReturningUser`), email signup (`showEmailSignupPopup`, `saveEmailSubscriber`), site-spanning guide functions (`navigateToPage`, `scrollToSection`, `highlightContent`, `scrollToContact`), cross-session memory via `voice_sessions` DB table, page-aware context (sends `page_change` events on navigation), session persistence via localStorage, auto-reconnect with exponential backoff (max 5 retries) as safety net for unexpected closures, text message queuing (`_pendingTextMessages`) to avoid disrupting active model speech, and AudioWorklet-based PCM capture/playback at 16kHz in / 24kHz out.
    ...
    -   **Site Guide**: Agent Friday can navigate pages, scroll to sections, and highlight content with a pulsing cyan/purple glow effect (`.friday-highlight` CSS class). Key sections have `id` and `data-section` attributes for targeting.
