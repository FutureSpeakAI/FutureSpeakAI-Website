...
-   **Voice Agent (Friday)**: Gemini 2.5 Flash native audio voice agent via WebSocket proxy (`server/gemini-live.ts`), model `models/gemini-2.5-flash-native-audio-latest`. Features: Kore voice, two-phase connection (Phase 1: greeting via `clientContent` → `generationComplete` → close; Phase 2: auto-reconnect in listening mode for bidirectional audio), function calling for name capture (`saveUserName`), returning-user recognition (`confirmReturningUser`), email signup (`showEmailSignupPopup`, `saveEmailSubscriber`), cross-session memory via `voice_sessions` DB table, page-aware context (sends `page_change` events on navigation), session persistence via localStorage, auto-reconnect on any Gemini WS closure, and AudioWorklet-based PCM capture/playback.
