...
7. **Capabilities** — 8 cards: voice, vision + SOC, parallel agents (5 concurrent + Agent Office), memory, coding (18+ connectors, 55+ tools), Relationship Intelligence (Relationship Graph), Communications Intelligence (context-aware drafting), meetings (trust-annotated)
7. **Additional Capabilities** — 26 cards including PageIndex, GitLoader, SOC, Trust-Annotated Meeting Prep, Predictive Intelligence, Agent Office, Superpowers Ecosystem (registry + sandbox + store), Project Awareness, Workflow Recorder, Git Analysis Suite (5 modules), Commitment Tracker, Daily Briefings, Context Stream, Unified Inbox, Agent Network, State Export/Import, Capability Gap Detection, Personality Calibration
...
9. **Under the Hood** — Model Orchestra (8 providers with Nano Banana 2 image gen, DALL-E 3 fallback), World Monitor (17 domains), Privacy/Security (18 bullets including Relationship Graph integrity + local-only + personality calibration + consent gate + superpower sandboxing), Tech Stack (18 entries including Relationship Intelligence, Workflows, Context Stream + Graph, IPC Bridge rows)
